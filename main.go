...
func defence(name, class string) string {
	if class == "warrior" {
		return fmt.Sprintf("%s блокировал %d урона.", name, 10+randNum(5, 10))
	}
	if class == "mage" {
		return fmt.Sprintf("%s блокировал %d урона.", name, 10+randNum(-2, 2))
	}
	if class == "healer" {
		return fmt.Sprintf("%s блокировал %d урона.", name, 10+randNum(2, 5))
	}
	return "неизвестный класс персонажа"
...
// обратите внимание на "if else" и на "else"
func special(charName, class string) string {
	if class == "warrior" {
